...
setValues = (curr, data) => {
  ...
  console.log('curr',curr);
    ...
    this.setState({...this.state, cryptos: {...this.state.cryptos,
										    		[curr]:{...data}
										    	}})
...
displayData = (current, from, tsym, fsym) => {
  ...
  componentWillMount = () =>{
  	...
  	let updatedState = {...this.state.cryptos}

  	this.props.stocks.filter((val)=>{
  	...
  	this.setState({...this.state, cryptos: {...updatedState}})
  ...
  componentDidMount = () => {
  	...
  	this.subscripts = this.props.stocks.map((val)=>{
  		let newval = `5~CCCAGG~${val}~USD`
    ...
    this.socket.emit('SubAdd', { subs: this.subscripts } );               
  ...
  componentWillUnmount(){
   ...
   console.log('i m unmounting')
    this.socket.emit('SubRemove', { subs: this.subscripts });
	...
	render(){
		...
		// const titles = ['Bitcoin','Ethereum','Eos','Tronix','Litecoin','Ripple'];

		return (

       
       
				<Grid fluid style={{marginTop:"60px" }}>
         {!this.props.isStock?  <Redirect to='/userchoice'/>: null}
					  <Row>
					  ...
					  {
					  		console.log(this.state.cryptos),
					   		Object.keys(this.state.cryptos).map((val,index) =>{

					   			return (<Insidelayout key={index} 
					   				shortname={val} 
					   				

					   				Price={this.state.cryptos[val].PRICE}
					   				Change={this.state.cryptos[val].CHANGE24HOURPCT}

					   				Open={this.state.cryptos[val].OPEN24HOUR}
					   				High={this.state.cryptos[val].HIGH24HOUR}
					   				Low={this.state.cryptos[val].LOW24HOUR}

					   				Vol ={this.state.cryptos[val].VOLUME24HOUR}
					   				

					   				Market={this.state.cryptos[val].MARKET}
...
const stateToProps = state =>{
	...
	return {
		stocks:state.selected_stock,
    isStock : state.selected_stock.length !== 0
	}
}

const mapDispatchToProps = dispatch =>{
    return {
      resetStocks: ()=> dispatch(actionTypes.resetStockSelected())
    }
}


export default connect(stateToProps,mapDispatchToProps)(layout);